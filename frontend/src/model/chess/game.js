...
export default class extends Game {
  ...
  static arrangePieces(board, plr1, plr2) {
    ...
    let id = 0;
    for (let x = 0; x < 8; x++) {
      if (color2) board.addPiece(x, 1, new Pieces.Pawn(id++, color2, 1));
      if (color1) board.addPiece(x, 6, new Pieces.Pawn(id++, color1, -1));
    }
    ...
    for (let y = 0; y < 8; y += 7) {
      ...
      if (color) {
        ...
        board.addPiece(
          plr1.color === "white" ? 3 : 4,
          y,
          new Pieces.Queen(id++, color)
        );
        board.addPiece(
          plr1.color === "white" ? 4 : 3,
          y,
          new Pieces.King(id++, color)
        );
  ...
  validateMove(plr, move) {
    ...
    // get all valid moves for piece in "source" cell
  ...
  commitMove(plr, move) {
    ...
    if (move.promotion != null) {
      // pawn got to the end of the board
      this.board.removePiece(move.piece, move.from.x, move.from.y);
      move.promotion.newPiece = new Pieces.Queen(
        this.nextPieceId++,
        move.piece.color
      );
      this.board
        .cell(move.to.x, move.to.y)
        .pieces.push(move.promotion.newPiece);
    } else {
      // no capture and no promotion
      console.log("moving piece");
      console.log(move.piece);
      console.log("from (" + move.from.x + "," + move.from.y + ")");
      console.log("to (" + move.to.x + "," + move.to.y + ")");
      // pawn direction
      ...
      if (move.castling != null) {
        this.board.movePiece(
          move.castling.piece,
          move.castling.from.x,
          move.castling.from.y,
          move.castling.to.x,
          move.castling.to.y
        );
      }
  ...
  *getValidMoves(plr, piece, from) {
    // generate all valid moves for piece in "from" cell position
    if (piece == null || piece.color != plr.color) return;

    // get piece valid moves in board
    for (const move of piece.getValidMoves(from, this.board, this.log)) {
      // move must not check king

      // TODO: check this
      //if (!this.willKingBeInCheck(plr, Object.assign(move, { from, piece })))
      yield move;
    }
  }
  ...
  isCastlingPossible(plr, castlingMove) {
    if (this.isKingInCheck(plr)) return false;
  ...
  undoLastMove() {
    ...
    if (move.capture != null) {
      const pieces = this.board.cell(move.capture.cell.x, move.capture.cell.y)
        .pieces;
  ...
  isInsufficientMaterial() {
    ...
    const hasOnlyKingAndBishop = (pp) =>
      pp.length === 2 &&
      pp.some((p) => p.piece.type === "king") &&
      pp.some((p) => p.piece.type === "bishop");

    const hasOnlyKingAndKnight = (pp) =>
      pp.length === 2 &&
      pp.some((p) => p.piece.type === "king") &&
    ...
    return (
      (hasOnlyKing(white) && hasOnlyKing(black)) ||
      ...
      (hasOnlyKingAndBishop(white) &&
        hasOnlyKingAndBishop(black) &&
        bishopCellColor(white) === bishopCellColor(black))
    );
  }
...
export function invColor(color) {
